...
if (!function_exists('curl')) {
    ...
    function curl($data)
    {
        ...
        $url = '10.1.6.60:4444/agencyunaitas/req';

        // 10.1.6.60:4444/agencyunaitas/req
...
if (!function_exists('signUpMail')) {

    function signUpMail($email, $subject, $message)
    {
        $time = Carbon::now();
        ...
        $array = ["username" => "esb",
            ...
            "msgType" => "1200",
            "processingCode" => "121000",
            "transactionType" => "EMAIL",
            "tranId" => hash('CRC32', $time . rand(145, 100544)),
            "subject" => $subject,
            "msg" => $message,
            "emailRecipient" => $email,
            "channel" => "USSD/APP/ADAPTOR/PORTAL"
        ];
...
if (!function_exists('status_label')) {
    ...
    function status_label($status)
    {
        ...
        switch ($status) {
            case in_array($status, ['correction', 'partially-paid', 'expired']):
                return badge('warning', title_case($status));
            case in_array($status, ['active', 'published', 'verified', 'complete', 'confirmed', 'valid', 'live', 'completed', 'resolved', 'success']):
            ...
            case in_array($status, ['disable', 'failed', 'pending']):
...
if (!function_exists('feed')) {
     ...
     * @param \App\Models\User|null $cause
     * @return \App\Models\Feed
     */
    function feed($title, $description, $type, $scope, $user, $cause = null)
    {
//        return \App\Models\Feed::add($user,$type,$scope,$title,$description,$cause);
    }
}
...
if (!function_exists('generate_ticket_no')) {
    ...
    {
        $token = 'T-AA00001';
        $last = \Illuminate\Support\Facades\DB::table('tickets')->latest('id')->lock()->first();
        if ($last) {
            $token = ++$last->ref;
        }
        return $token;
...
if (!function_exists('business_types')) {
    /**
     * @return \App\Models\BusinessType[]|\Illuminate\Database\Eloquent\Collection
     */
...
if (!function_exists('institution_check')) {

    function institution_check($id)
    {
        if ($id == 1) {
            $check = "Primary";
        } else {
...
if (!function_exists('institution_name')) {
    ...
    {
        return $institutionType = \App\Institution::where('id', $id)->first()->name;
    }
}
...
if (!function_exists('agent_name')) {
    ...
    {
            return \App\Agents::where('id', $id)->first()->agent_name;
    }
}

if (!function_exists('agent_code')) {

    function agent_code($id)
    {
        return \App\Agents::where('id', $id)->first()->agent_code;
    }
}

if (!function_exists('agent_type')) {

    function agent_type($id)
    {
        return \App\AgentType::where('id', $id)->first()->name;
    }
}

if (!function_exists('agents_code')) {

    function agents_code($id) {
        return \App\Agents::where('id', $id)->first()->agent_code;
    }
}

if (!function_exists('agent_classification')) {

    function agent_classification($id)
    {
        return $classification = \App\AgentClassification::where('id', $id)->first()->name;
    }
}

if (!function_exists('transaction_type')) {

    function transaction_type($id)
    {
        $tranType = '';
...
if (!function_exists('agent_uq_id')) {

    function agent_uq_id()
    {
        return session('id');
    }
}
...
if (!function_exists('float_balance')) {

    function float_balance()
    {
        return session('float_account');
    }
}
...
if (!function_exists('status_names')) {

    function status_names($id)
    {
        $name = "";
        ...
        if ($id == 4) {
            return $name = "Updated (verify)";
        }

        if ($id == 5) {
            return $name = "Devie Reset";
        }

        if ($id == 6) {
            return $name = "Pin Reset";
        }
...
if (!function_exists('agent_type_name')) {

    function agent_type_name($id)
    {
        $name = '';

        if ($id == 1) {
            return $name = 'Staff Agent';
        }
...
if (!function_exists('account_label')) {

    function account_label($code)
    {
        $name = 'Account';

        if ($code == 6021) {
            return $name = 'AGENCY COMMISSION ACCOUNT';
        }

        if ($code == 6024) {
            return $name = 'AGENT FLOAT ACCOUNT';
        }
...
if (!function_exists('branch_code_name')) {

    function branch_code_name($code)
    {
        return Branch::where('branch_code', $code)->pluck('name')->first();
    }
}
...
if (!function_exists('logged_user')) {
    ...
    {
        return User::where('id', $id)->pluck('first_name')->first();
    }
}
